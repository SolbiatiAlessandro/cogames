"""Competition map, FIXED seed 42, NO CLIPS, 5000 steps — V3: targeted gear retention.

Key insight from V1 and Long4 experiments:
- V1 (no penalties): good navigation (55% move success) but 0 alignment (agents gain+lose
  aligner equally at 0.62/agent). No feedback for gear loss.
- Long4 (wrong-gear penalties): slow navigation (max_steps=16 at e28) because penalties
  on scrambler/miner/scout.gained punish EXPLORATION near stations.

V3 solution: instead of penalizing wrong gear PICKUP (which hinders exploration), penalize
aligner LOSS directly. The stat 'aligner.lost' fires only when agent REPLACES aligner gear
by walking over another station. Combined with strong aligner.gained bonus, this teaches
"get aligner → keep aligner" without penalizing approaching other areas.
...
for agent_cfg in config.game.agents:
    ...
    rewards["aligner_lost_penalty"] = reward(
        stat("aligner.lost"),
        weight=-10.0,
    )
