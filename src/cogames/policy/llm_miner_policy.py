...
class LLMMinerPolicyImpl(MinerSkillImpl, StatefulPolicyImpl[LLMMinerState]):
    ...
    def __init__(
        ...
        self._base_return_load = return_load
        self._agent_id = agent_id
        if shared_map is not None and hasattr(shared_map, 'active_miner_ids'):
    ...
    def _effective_return_load(self) -> int:
        ...
        if sm is None or not hasattr(sm, 'active_miner_ids'):
            return self._base_return_load
        n_miners = len(sm.active_miner_ids)
        if n_miners >= 3:
            return self._base_return_load
        return max(15, self._base_return_load * n_miners // 3)
    ...
    def _copy_with(self, state: LLMMinerState, base: MinerSkillState) -> LLMMinerState:
        ...
        return replace(
            ...
            depleted_extractors=sm.depleted_extractors if sm and hasattr(sm, "depleted_extractors") else set(base.depleted_extractors),
    ...
    def _update_progress(self, obs: AgentObservation, state: LLMMinerState) -> None:
        ...
        sm = self._shared_map
        _fj = sm.known_friendly_junctions if sm is not None else set()
        stationary_on_valid_target = (
            (state.current_skill == "mine_until_full" and _adjacent_to(self._active_extractors(state)))
            or (state.current_skill == "deposit_to_hub" and (_adjacent_to(state.known_hubs) or _adjacent_to(_fj)))
    ...
    def _scripted_skill_choice(self, obs: AgentObservation, state: LLMMinerState) -> tuple[str, str]:
        ...
        if team_scarce and not self._active_extractors_for_element(state, team_scarce):
            return "explore", f"scripted: team needs {team_scarce} but no active extractors known"
    ...
    def _plan_skill(self, obs: AgentObservation, state: LLMMinerState) -> None:
        ...
        if self._planner is None:
            ...
            prompt = build_llm_miner_prompt(
                carried_total=self._carried_total(obs),
                return_load=self._effective_return_load,
                has_miner=has_miner,
    ...
    def _maybe_finish_skill(self, obs: AgentObservation, state: LLMMinerState) -> None:
        ...
        if state.current_skill == "gear_up" and has_miner:
            ...
            state.current_skill = None
        elif state.current_skill == "gear_up" and state.skill_steps >= max(self._stuck_threshold * 2, self._stuck_threshold * 5 // (1 + state.gear_contamination_count)):
            state.gear_up_approach_rotation = (state.gear_up_approach_rotation + 1) % 4
            state.gear_contamination_count += 1
            if self._shared_map is not None and hasattr(self._shared_map, 'move_blocked_cells'):
                state.blocked_cells.difference_update(self._shared_map.move_blocked_cells)
                self._shared_map.move_blocked_cells.clear()
            self._event(state, f"gear_up timed out after {state.skill_steps} steps (contam_count={state.gear_contamination_count})")
            state.current_skill = None
        elif state.current_skill == "mine_until_full" and state.skill_steps >= self._stuck_threshold * 5:
