...
class LLMAlignerPolicyImpl(AlignerPolicyImpl, StatefulPolicyImpl[LLMAlignerState]):
    ...
    def _plan_skill(self, obs: AgentObservation, state: LLMAlignerState) -> None:
        ...
        if skill == "get_heart" and self._shared_map is not None:
            ...
            if already_getting >= max_queue:
                if has_aligner and len(state.known_friendly_junctions) >= 3:
                    skill = "defend"
                    reason = f"heart queue full: {already_getting}/{max_queue} en route, ~{available_hearts} avail — defending instead"
                else:
                    skill = "explore"
                    reason = f"heart queue full: {already_getting}/{max_queue} en route, ~{available_hearts} avail — exploring instead"
        if skill == "get_heart" and self._shared_map is not None:
    ...
    def _step_impl(self, obs: AgentObservation, state: LLMAlignerState) -> tuple[Action, LLMAlignerState]:
        ...
        # ── Periodic contamination cell expiry: prevent navigation degradation ──
        _CONTAMINATION_EXPIRY_INTERVAL = 500
        if state.global_step % _CONTAMINATION_EXPIRY_INTERVAL == 0 and state.contamination_avoid_cells:
