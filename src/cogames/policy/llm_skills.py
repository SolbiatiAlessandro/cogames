    ...
    def _predict_stations_from_hubs(self, state: MinerSkillState) -> None:
        ...
        for hub in hub_set:
            ...
            for dr, dc in _HUB_HAZARD_STATION_OFFSETS:
                pos = (hub[0] + dr, hub[1] + dc)
                state.known_hazard_stations.add(pos)
                state.blocked_cells.add(pos)
            miner_pos = (hub[0] + _HUB_MINER_STATION_OFFSET[0], hub[1] + _HUB_MINER_STATION_OFFSET[1])
            state.known_miner_stations.add(miner_pos)
            state.blocked_cells.add(miner_pos)
            logger.info("PREDICTED_STATIONS hub=%s hazards=%s miner=%s",
                        hub, [(hub[0]+dr, hub[1]+dc) for dr, dc in _HUB_HAZARD_STATION_OFFSETS], miner_pos)
