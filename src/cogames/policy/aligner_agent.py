...
class SharedMap:
    ...
    def __init__(self) -> None:
        ...
        # Per-element extractor positions (for balanced miner; survives death/respawn)
        self.known_carbon_extractors: set[Coord] = set()
        self.known_oxygen_extractors: set[Coord] = set()
        self.known_germanium_extractors: set[Coord] = set()
        self.known_silicon_extractors: set[Coord] = set()
...
class AlignerPolicyImpl(StatefulPolicyImpl[AlignerState]):
    ...
    def _align_neutral(self, obs: AgentObservation, state: AlignerState, current_abs: Coord) -> tuple[Action, AlignerState]:
        ...
        target_abs = self._nearest_known(current_abs, unclaimed_alignable)
        if target_abs is None:
        ...
        if target_abs is None and state.known_enemy_junctions:
            ...
            unclaimed_enemy = enemy_alignable - other_claimed
