"""Standalone CNN + LSTM policy for tournament submission."""
...
def coordinates(observations: torch.Tensor, dtype: torch.dtype = torch.uint8):
...
class TutorialPolicyNet(nn.Module):
    ...
    def forward(
        ...
        hidden = hidden.view(segments, bptt_horizon, -1)
...
class TutorialPolicy(MultiAgentPolicy):
    short_names = ["rl_tutorial_standalone"]
